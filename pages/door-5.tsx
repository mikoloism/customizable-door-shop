...
import style from 'styles/door-5-variables.module.scss';
...
export default function HomePage() {
    ...
    const fixed5 = {
        ...
        setValue: null as any,
        onChange(event: React.ChangeEvent<HTMLInputElement>) {
            fixed5.setValue(() => event.target.value);
        },
        onKeyUp(event: KeyboardEvent<HTMLInputElement>) {
            if (event.key === 'Enter') {
                wrapper.current!.style.setProperty(
                    '--panel-5-fixed-height',
                    `${fixed5.value}px`
                );
                wrapper.current!.style.setProperty(
                    '--panel-5-relative-height',
                    `0`
                );
                relative5.setValue(0);
            }
        },
    };
    ...
    const relative5 = {
        ...
        setValue: null as any,
        onChange(event: React.ChangeEvent<HTMLInputElement>) {
            relative5.setValue(() => event.target.value);
        },
        onKeyUp(event: KeyboardEvent<HTMLInputElement>) {
            if (event.key === 'Enter') {
                wrapper.current!.style.setProperty(
    ...
    [fixed5.value, fixed5.setValue] = useState<number>(0);
    [relative5.value, relative5.setValue] = useState<number>(1);
            ...
            <div className="flex flex-col flex-wrap gap-y-9 fixed right-8 top-10 bg-blue-100 p-9 rounded-md">
                ...
                <div className="flex flex-row flex-wrap w-full h-32 gap-x-3 bg-blue-200 p-9 rounded-md">
                    <label className="w-32 h-9 flex flex-col gap-y-2">
                        ...
                        <input
                            type="number"
                            onKeyUp={fixed5.onKeyUp}
                            onChange={fixed5.onChange}
                            value={fixed5.value}
                            id="width"
                            className="w-32 h-8 p-4 rounded-md bg-blue-100"
                        />
                    ...
                    <label className="w-32 h-9 flex flex-col gap-y-2">
                        <span>(5) Relative :</span>
                        <input
                            type="number"
                            onKeyUp={relative5.onKeyUp}
                            onChange={relative5.onChange}
                            value={relative5.value}
                            id="width"
                            className="w-32 h-8 p-4 rounded-md bg-blue-100"
                        />
                    </label>
            ...
            <div
                ...
                <Group
                    ...
                    <Group
                        ...
                        <ProfileSeparator
                            axis="horizontal"
                            position={{ top: 'var(--separator-4-top)' }}
                        />

                        <Panel
                            position={{ top: 'var(--panel-5-top)' }}
                            style={{
                                height: 'var(--panel-5-height)',
                                width: '100%',
                            }}
                        />
                    </Group>
